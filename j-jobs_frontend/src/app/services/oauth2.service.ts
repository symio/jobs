...
export class OAuth2Service {
  ...
  login(credentials: OAuth2Credentials): Observable<OAuth2TokenResponse> {
    ...
    return this.authenticate(
      `${this.apiBaseUrl}/authorize/token`,
      body.toString(),
      headers,
    );
  }
  ...
  refreshToken(): Observable<OAuth2TokenResponse> {
    const headers = this.buildRequestHeaders(false);
    ...
    return this.authenticate(
      `${this.apiBaseUrl}/authorize/refresh`,
      body,
      headers,
    );
  }
  ...
  logout(): Observable<void> {
    ...
    return this.http
      .post<void>(`${this.apiBaseUrl}/authorize/cleanup`, body)
      .pipe(
  ...
  private authenticate(
    ...
    body: any,
    headers: HttpHeaders,
  ): Observable<OAuth2TokenResponse> {
    return this.http.post<OAuth2TokenResponse>(url, body, { headers }).pipe(
  ...
  private handleAuthError(err: any): Observable<never> {
    console.error(
      '[OAuth2Service] Erreur d’authentification critique, déconnexion forcée:',
      err,
    );
    this.removeStoredToken();
    this.removeStoredUserInfo();
  ...
  private extractAndStoreUserInfo(token: string): void {
    try {
      ...
      const userInfo: UserInfo = {
        ...
        displayName: payload.display_name || payload.client_id || payload.sub,
        scope: payload.scope || '',
      ...
      console.error(
        "[OAuth2Service] Erreur lors de l'extraction des infos utilisateur:",
        error,
      );
    }
  }
   ...
   */
  private decodeTokenExpirations(): {
    exp: number;
    originallyExpires: number;
  } | null {
    try {
      const token = this.getStoredToken();
